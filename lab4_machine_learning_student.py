...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    iterator=IDandRatingsTuple[1]
    count=0
    ratingsum=0
    for rating in iterator:
        count+=1
        ratingsum+=rating
    ratingavg=float(ratingsum)/count
    countandavg=(count,ratingavg)
    ans=(id,countandavg)
    return (ans)
...
def fun(x,y):
    ratingtuple=tuple()
    if type(x) != type(ratingtuple):
        x=(x,)
    if type(y) !=type(ratingtuple):
        y=(y,)
    ratingtuple=x+y
    return (ratingtuple)
movieIDsWithRatingsRDD = (ratingsRDD
                          .map(lambda (u,mid,r):(mid,r)).groupByKey())#.map(lambda (x,y):(x,iter(y))))
...
movieNameWithAvgRatingsRDD = (moviesRDD
                              .join(movieIDsWithAvgRatingsRDD).map(lambda (mid,(title,(count,rating))):(rating,title,count)))
...
def computeError(predictedRDD, actualRDD):
    ...
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD).map(lambda (x,(r1,r2)):(math.pow((r1-r2),2))))
    #print squareErrorsRDD.take(3)
...
# TODO: Replace <FILL IN> with appropriate code
trainingcount=trainingRDD.count()
trainingAvgRating = trainingRDD.map(lambda (u,m,r):r).reduce(lambda x,y: x+y)/trainingcount
print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda (uid,mid,r):(uid,mid,trainingAvgRating))
...
myRatedMovies = [
     (myuserID,805,5),(myuserID,993,1),(myuserID,1039,4),(myuserID,503,3),(myuserID,587,3),(myuserID,1088,5),(myuserID,1171,4),(myuserID,539,2),
    (myuserID,817,1),(myuserID,551,4)]
...
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda x:x[0] not in myRatedMovies)).map(lambda (mid,title):(myuserID,mid))
...
predictedRDD = predictedRatingsRDD.map(lambda (m,t,r):(m,r))
