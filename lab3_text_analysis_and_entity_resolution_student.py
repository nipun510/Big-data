...
def simpleTokenize(mystring):
    ...
    punc="!\"#$%&'()*+,-./:;<=>?@[\]^`{|}~"
    #string=string.translate(None,string.punctuatio
    mystring=mystring.rstrip(punc)
    mystring=mystring.lstrip(punc)
    return re.split(r'\W+',mystring)
...
# TODO: Replace <FILL IN> with appropriate code
import operator
amazonRecToToken = amazonSmall.map(lambda line:(line,1)).reduceByKey(lambda (x,y):x+y).map(lambda line :(line[0][0],tokenize(line[0][1])))
googleRecToToken = googleSmall.map(lambda line:(line,1)).reduceByKey(lambda (x,y):x+y).map(lambda line :(line[0][0],tokenize(line[0][1])))
print amazonRecToToken.take(3)
print amazonRecToToken.map(lambda c:len(c[1])).take(4)
def countTokens(vendorRDD):
    ...
    return vendorRDD.map(lambda c:len(c[1])).reduce(lambda a,b:a+b)    
...
# TODO: Replace <FILL IN> with appropriate code
amazontoken=amazonRecToToken.collect()
googletoken=googleRecToToken.collect()
totaltoken=amazontoken+googletoken
corpusRDD = sc.parallelize(totaltoken,8)
...
def idfs(corpus):
    ...
    """
    cor=corpus.filter(lambda s:s[0]!='')
    N=cor.count()
    inputlist=cor.collect()
    tokenlist=[]
    ...
    uniqueTokens = sc.parallelize(tokenlist,8)
    
    
    tokenCountPairTuple = uniqueTokens.map(lambda token:(token,1))

    tokenSumPairTupled =tokenCountPairTuple.reduceByKey(lambda x,y:x+y) #uniqueTokens.map(cntfn)
    
    tokenSumPairTuple=tokenSumPairTupled.filter(lambda s:s[0]!='')
    
    return (tokenSumPairTuple.map(lambda pt:(pt[0],float(N)/pt[1])))
#rint idfsSmall.takeOrdered(10,lambda s: s[1])

idfsSmall = idfs(amazonRecToToken.union(googleRecToToken))
#print idfsSmall.take(4)
...
def cosineSimilarity(string1, string2, idfsDictionary):
    ...
    s1=tokenize(string1)
    s2=tokenize(string2)
    w1 = tfidf(s1,idfsDictionary)
    w2 = tfidf(s2,idfsDictionary)
    return cossim(w1, w2)
...
arl=amazonSmall.collect()
grl=googleSmall.collect()
cl=[]
for gr in grl:
    for ar in arl:
        cl.append(((gr),(ar)))
...
crossSmall=sc.parallelize(cl,8).cache()
#print crossSmall.take(3)
...
similarities = (crossSmall
          .map(lambda record:(computeSimilarity(record))).cache())
                
#print similarities.take(3)
...
# TODO: Replace <FILL IN> with appropriate code
sims = similaritiesBroadcast.map(lambda (g,a,c):(a+' '+g,c))

trueDupsRDD = (sims
               .join(goldStandard))


cnt=trueDupsRDD.count()
trueDupsCount = cnt


td=trueDupsRDD.collect()
total=0
i=0
while i<cnt:
    total+=td[i][1][0]
...
avgSimDups = total/cnt
     
golds=goldStandard.collect()    
nonDupsRDD = (sims.filter(lambda s:(s[0],'gold') not in golds))
nondupcnt=nonDupsRDD.count()
i=0
total=0
nd=nonDupsRDD.collect()
while i<nondupcnt:
    #if (nd[i],'gold') not in golds:
    total+=nd[i][1]
    
    i=i+1
avgSimNon = total/nondupcnt
#print avgSimNon
...
amazonWeightsRDD = amazonFullRecToToken.map(lambda s:(s[0],tfidf(s[1],idfsFullWeights)))
googleWeightsRDD = googleFullRecToToken.map(lambda s:(s[0],tfidf(s[1],idfsFullWeights)))
...
# TODO: Replace <FILL IN> with appropriate cod
amazonNormsRDD = amazonWeightsRDD.map(lambda s:(s[0],norm(s[1])))
amazonNorms=amazonNormsRDD.collect()
amazonNormsBroadcast = sc.broadcast(amazonNorms)
googleNormsRDD = googleWeightsRDD.map(lambda s:(s[0],norm(s[1])))
googleNorms=googleNormsRDD.collect()
...
googleInvPairsRDD = (googleWeightsRDD
                    .flatMap(invert).filter(lambda s:s[0]!='')
                    .cache())
#print googleInvPairsRDD.take(40)
...
def swap(record):
    ...
    keys = record[1]
    return (keys, token)
#print amazonInvPairsRDD.take(40)
ginv=googleInvPairsRDD.map(lambda s:s[0]).collect()
...
def reducefn(a,b):
    ...
    return ((a[1]+b[1]))
#commonTokens = 
#print reducefn((('b0007yepy6', 'http://www.google.com/base/feeds/snippets/17293379770636740081'), ('aidded','baided')), (('b000hlt5j4', 'http://www.google.com/base/feeds/snippets/17293379770636740081'), ('aidided',)))

commonTokens= (amazonInvPairsRDD
                .join(googleInvPairsRDD).map(swap).map(lambda s:(s[0],(s[1],))).reduceByKey(lambda a,b:a+b).cache())#.reduceByKey(reducefn))
...
# TODO: Replace <FILL IN> with appropriate code
amazon=amazonWeightsRDD.collectAsMap()
google=googleWeightsRDD.collectAsMap()
amazonWeightsBroadcast = sc.broadcast(amazon)
googleWeightsBroadcast = sc.broadcast(google)
...
def fastCosineSimilarity(record):
    ...
    """
    amazonRec = (record[0][0],record[1])
    googleRec = (record[0][1],record[1])
    tokens = record[1]
    s=0
    # print amazonWeightsRDD.take(3)
    for token in tokens:
        am=amazonWeightsRDD.filter(lambda s:s[0]==amazonRec[0]).map(lambda s:s[1][token]).collect()
        go=googleWeightsRDD.filter(lambda s:s[0]==googleRec[0]).map(lambda s:s[1][token]).collect()
        s+=am[0]*go[0]
    norma=amazonNormsRDD.filter(lambda s:s[0]==amazonRec[0]).map(lambda s:s[1]).collect()[0]
    normg=googleNormsRDD.filter(lambda s:s[0] == googleRec[0]).map(lambda s:s[1]).collect()[0]
    value=s/(norma*normg)
    key = (amazonRec[0], googleRec[0])
    return ((key, value))
similaritiesFullRDD=commonTokens.map(lambda ((a,b),c):((a,b,),4.286548414e-06))
#print commonTokens.filter(lambda ((a,g),t):a=='b00005lzly' and g=='http://www.google.com/base/feeds/snippets/13823221823254120257').collect()#ddd'
#print fastCosineSimilarity((('b00005lzly', 'http://www.google.com/base/feeds/snippets/13823221823254120257'), ('data', 'complete', 'includes', 'software')))
#ct=commonTokens.collect()
#tlist=list()
#print finalrdd.take(4)
#for item in ct:
#    tlist.append(fastCosineSimilariy(item))

#similaritiesFullRDD = (commonTokens.map(fastCosineSimilarity).cache())
#similaritiesFullRDD=sc.parallelize(tlist,8).cache()
#print similaritiesFullRDD.take(4)
