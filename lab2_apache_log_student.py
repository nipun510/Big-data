...
#dailyHosts = dayToHostPairTuple.groupByKey().map(lambda (x,y):(x,len(list(y)))).cache()

#print dailyHosts.count()

dailyHostsList=[(1, 2582), (3, 3222), (4, 4190), (5, 2502), (6, 2537), (7, 4106), (8, 4406), (9, 4317), (10, 4523), (11, 4346), (12, 2864), (13, 2650), (14, 4454), (15, 4214), (16, 4340), (17, 4385), (18, 4168), (19, 2550), (20, 2560), (21, 4134), (22, 4456)]

dailyHosts=sc.parallelize(dailyHostsList,8).cache()
...
#dailyHosts = dayHostCount.takeOrdered(count)
...
daysWithHosts = dailyHosts.map(lambda (a,b):a).collect()
hosts = dailyHosts.map(lambda (a,b):b).collect()
...
def uqhosts(hosts):
    ...
    for host in hosts:
        if host not in counts:
            ...
            counter=counter+1
    return counter   
daytotalhosts=dayAndHostTuple.groupByKey().map(lambda (x,y):(x,len(list(y)))).takeOrdered(21)
thdic=dict()
for day,host in daytotalhosts:
    thdic[day]=host

#def avgcnt(value):
     
        
groupedByDay = dayAndHostTuple.groupByKey().map(lambda (k,x):(k,uqhosts(x)))
#print groupedByDay.collect()

sortedByDaylist = groupedByDay.takeOrdered(21)
sortedByDay=sc.parallelize(sortedByDaylist,8)

avgDailyReqPerHost = (sortedByDay.map(lambda (x,y):(x,thdic[x]/y) ).cache())
...
badUniqueEndpoints = badEndpoints.map(lambda x:(x,1)).reduceByKey(lambda x,y:x+y).map(lambda (x,y):x)
...
errDateSortedList = (errDateSum.takeOrdered(21)
                 )
errDateSorted=sc.parallelize(errDateSortedList,8).cache();

errByDate = errDateSorted.takeOrdered(21)
...
#print hourRecordsSum.collect()
hourRecordsSortedList = hourRecordsSum.takeOrdered(24)
                     

hourRecordsSorted=sc.parallelize(hourRecordsSortedList,8).cache()
errHourList = hourRecordsSorted.takeOrdered(24)
