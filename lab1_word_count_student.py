...
# TODO: Replace <FILL IN> with appropriate codde
def unwords(x):
    count=0
    for p in x:
        count=count+1
...
uniqueWords = unwords(wordCountsCollected)
print uniqueWords
...
def wordCount(wordListRDD):
    ...
    """
    from operator import add
    wc=wordListRDD.map(lambda x:(x,1)).reduceByKey(add)
    return wc
...
shakespeareWordsRDD = shakespeareRDD.flatMap(wordcount)
shakespeareWordCount = 928908
...
# TODO: Replace <FILL IN> with appropriate code
shakeWordsRDD = shakespeareWordsRDD.map(lambda x:x)
...
sw=wordCount(shakeWordsRDD)
sw.collect()
top15WordsAndCounts=sw.takeOrdered(15,lambda (x,y):-y)
